...
pub struct SessionResponse {
    ...
    /// Set when this session was auto-generated by a recurring rule, so the
...
/// by the caller (custom code not taken / existing code exists and, for the
/// HTTP path, not locked to another active recurring rule) — `create_session_row`
/// just does the mechanical insert/update inside its transaction.
pub enum ShortlinkDirective {
    None,
    Auto,
    Custom(String),
    Existing(String),
...
/// Everything needed to insert one `sessions` row and (optionally) attach a
/// short link, shared by the manual HTTP `create_session` path and the
/// recurring-rule scheduler so the two never drift apart.
pub struct SessionSpec {
    ...
    pub starts_at: Option<DateTime<Utc>>,
    pub expires_at: DateTime<Utc>,
    /// Set by the scheduler for auto-generated occurrences; always `None`
...
/// all within the caller's transaction — any error propagates via `?` and the
/// caller's transaction rolls back automatically on drop, so no manual
/// compensating `DELETE FROM sessions` is needed here.
///
/// Runs inside `tx` rather than opening its own transaction because the
/// recurring-rule scheduler needs the row-claim, the session insert, the
/// short-link re-point, and the schedule advance to all commit atomically
/// together (see `services::recurring_scheduler`).
pub async fn create_session_row(
    ...
    let short_code = match spec.shortlink {
        ShortlinkDirective::None => None,
        ShortlinkDirective::Custom(code) => {
            sqlx::query(
            ...
            .execute(&mut **tx)
            .await
            .map_err(|e| AppError::Internal(format!("Failed to create short link: {}", e)))?;
            Some(code)
        }
        ShortlinkDirective::Existing(code) => {
            sqlx::query(
            ...
            .execute(&mut **tx)
            .await
            .map_err(|e| AppError::Internal(format!("Failed to attach short link: {}", e)))?;
            Some(code)
        }
        ShortlinkDirective::Auto => {
            let mut attempts = 0;
            let code = loop {
...
pub async fn create_session(
    ...
    let shortlink = match mode {
        ...
        "custom" => ShortlinkDirective::Custom(custom_code_to_use.unwrap()),
        "existing" => ShortlinkDirective::Existing(existing_code_to_use.unwrap()),
        _ => ShortlinkDirective::Auto,
    };

    let mut tx = state.db.begin().await?;

    let (session, created_short_code, token) = create_session_row(
        &mut tx,
        SessionSpec {
            location_id,
            batch_id,
